...
import {
  ...
  FormatDateEN,
  FormatDateHourBR,
} from '@lhelpers/DateFunctions.js';
...
import {
  DataItemsDetailsProps,
  ItemsDetailsProps,
} from '@pages/Finance/Details/types';
...
export const useGetFinanceID = () => {
  ...
  const getFinanceDataID = async () => {
    try {
      ...
      const response: DataItemsProps = await Api.get(
        `/core/v1/contas-financeiro?${params}`,
      );
      ...
      dispatch(
        ToastNotifyActions.toastNotifyShow(
          'Não foi possível recuperar estes lançamentos',
          true,
        ),
      );
    } finally {
...
export const useGetResume = () => {
  ...
  const getReleaseResume = async (filters: ItemProps) => {
    try {
      ...
      dispatch(
        ToastNotifyActions.toastNotifyShow(
          'Não foi possível buscar este resumo',
          true,
        ),
      );
    } finally {
  ...
  const getInstallments = async (filters: FiltersReleaseDataProps) => {
    try {
      ...
      Object.keys(filtersFormated).forEach(key => {
        const value = filtersFormated[key];
        // Codifica o valor na URL, especialmente importante para datas com caracteres especiais
        const encodedValue = encodeURIComponent(value);
        filtersParams += key + '=' + encodedValue + '&';
      });
      ...
      const itensOptimized = itens.map((item: ItemInstallmentsProps) => {
        ...
        const dataBaixaFormatada = item.dataBaixa
          ? FormatDateBR(item.dataBaixa)
          : null;
      ...
      dispatch(
        ToastNotifyActions.toastNotifyShow(
          'Não foi possível buscar as parcelas',
          true,
        ),
      );
    } finally {
...
export const useGetInstallmentsDetails = () => {
  const [isLoadingInstallmentsDetails, setIsLoadingInstallmentsDetails] =
    useState(false);
    ...
    try {
      ...
      if (itens.length > 0) {
        ...
        return {
          ...
          categoria: item.categoriaFinanceiro?.nomeCategoriaFinanceiro || null,
          idCategoriaFinanceiro:
            item.categoriaFinanceiro?.idCategoriaFinanceiro || null,
          // Tenta pegar idPessoaCliente de pessoaCliente.idPessoaCliente ou diretamente de idPessoaCliente
          idPessoaCliente:
            item.pessoaCliente?.idPessoaCliente || item.idPessoaCliente || null,
          ...
          categoriaFinanceiro: item.categoriaFinanceiro || null,
        };
      } else {
        dispatch(
          ToastNotifyActions.toastNotifyShow('Lançamento não encontrado', true),
        );
        return null;
      }
    } catch (error) {
    ...
    async (data: ItemProps, handleCallback: () => void) => {
      try {
        ...
        const response = await Api.post(
          `/core/v1/lancamentos-financeiro`,
          data,
        );
        ...
        dispatch(
          ToastNotifyActions.toastNotifyShow(
            'Não foi possível cadastrar este lançamento',
            true,
          ),
        );
      } finally {
  ...
  const updateRelease = useCallback(
    async (data: ItemProps, handleCallback: () => void) => {
      try {
        ...
        const errorMessage =
          responseMessage || 'Não foi possível ativar esta categoria';
    ...
    async (item: ItemProps, handleCallback: () => void) => {
      try {
        ...
        const response = await Api.put(
          `core/v1/parcelas-financeiro/inativar`,
          data,
        );
  ...
  const sendReleaseEmail = useCallback(
    async (data: DataEmailProps, handleCallback: () => void) => {
      try {
        ...
        dispatch(
          ToastNotifyActions.toastNotifyShow(
            'Não foi possível enviar este lançamento',
            true,
          ),
        );
      } finally {
...
export const useFinancialLoss = () => {
  ...
  const addFinancialLoss = useCallback(
    async (data: FinancialLossProps, handleCallback: () => void) => {
      try {
        ...
        dispatch(
          ToastNotifyActions.toastNotifyShow(
            'Não foi possível cadastrar a baixa financeira',
            true,
          ),
        );
      } finally {
  ...
  const removeFinancialLoss = useCallback(
    async (
      data: { idParcelaFinanceiro: number },
      handleCallback: () => void,
    ) => {
      try {
        setIsLoadingFinancialLoss(true);
        ...
        const response = await Api.put(
          `/core/v1/baixas-financeiro/inativar`,
          dataFinancialLoss,
        );
        ...
        dispatch(
          ToastNotifyActions.toastNotifyShow(
            'Não foi possível remover a baixa financeira',
            true,
          ),
        );
      } finally {
