				...
				<MovementTags>
					{publicacao.dataDivulgacaoDiarioEdicao && (
						<Tag background={colors.gray} key={1}>
							<TagText>
								Disponibilização em: {FormatDateBR(publicacao.dataDivulgacaoDiarioEdicao)}
							</TagText>
						</Tag>
					)}
					...
					{publicacao.dataPublicacaoDiarioEdicao && (
						<Tag background={colors.gray} key={2}>
							<TagText>
								Publicação em: {FormatDateBR(publicacao.dataPublicacaoDiarioEdicao)}
							</TagText>
						</Tag>
					)}
					...
					{publicacao.paginaInicial > 0 && publicacao.paginaFinal > 0 && (
						<Tag background={colors.gray} key={8}>
							<TagText>
								Páginas: {publicacao.paginaInicial || 0} a {publicacao.paginaFinal || 0}
							</TagText>
						</Tag>
					)}
				</MovementTags>
			...
			<Warp>
				...
				<Content>
					...
					<Value>
						...
						{deadline.diaInteiro ? '- Dia todo' : 'às ' + deadline.horaFormatada}
					</Value>
					<Label>Tipo</Label>
				...
				<Confirmation
					ref={confirmationRef}
					deadline={deadline}
					remove={() => props.navigation.navigate('Deadlines', {removeIndex: deadline.id})}
				/>
			</Warp>
		</Container>
	);
