...
import MovementActions from '@lstore/ducks/Movement';
...
import { BASE_URL, getLogin } from '@lservices/Api';
...
export default Movements = props => {
  ...
  const { loadingDeleteTrash, currentDayDeleteMovTrash } =
    useMovementsGetDeleteTrash();
  ...
  const movements = useSelector(state =>
    state.movements.data.map(movement => {
      if (!movementsRef[movement.id])
        movementsRef[movement.id] = new Animated.Value(1);
      ...
      folder.idTipoPasta == -2
        ...
        : {
            ...
            data: tribunals.map(tribunal => {
              return {
                nome: tribunal.nomeOrgaoJudiciario,
                id: tribunal.idOrgaoJudiciario,
              };
            }),
          };
  ...
  useEffect(
    () =>
      movements.forEach((move, i) => {
        if (!movementsRef[move.id])
          movementsRef[move.id] = new Animated.Value(1);
      }),
    [movements],
  );
    ...
    try {
      if (Platform.OS === 'android') {
        ...
        const status = await PermissionsAndroid.request(permission, {
          ...
          message:
            'O app precisa de acesso ao armazenamento para baixar e compartilhar arquivos.',
          buttonPositive: 'OK',
        });
          ...
          >
            {!item.lido && (
              <Tag background={colors.blue200}>
                <TagText>
                  {item.idTipoMovProcesso === -1 ? 'Não lido' : 'Não lida'}
                </TagText>
              </Tag>
            )}
            {item.idTipoMovProcesso === -1 && (
            ...
            {item.idTipoMovProcesso === -2 && (
              <>
                ...
                {item.palavrasChaves &&
                  Array.isArray(item.palavrasChaves) &&
                  [...item.palavrasChaves]
                    .filter(
                      keyword =>
                        keyword &&
