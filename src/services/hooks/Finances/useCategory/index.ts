  ...
  const getCategoryData = async (
    ...
    try {
      ...
      if (
        ...
        const filterParts = [];
        if (filtersData.type !== null && filtersData.type !== undefined) {
          filterParts.push(`IdsTipoCategoriaFinanceiro=${filtersData.type}`);
        }
        if (
          filtersData.situation !== null &&
          filtersData.situation !== undefined
        ) {
          filterParts.push(`ativo=${filtersData.situation}`);
        }
      ...
      const response: DataCategoryProps = await Api.get(url);
...
export const useCategory = () => {
  ...
  const saveCategory = async (
    ...
    try {
      ...
      const errorMessage =
        responseMessage || 'Não foi possível cadastrar esta categoria';
  ...
  const updateCategory = async (
    ...
    try {
      ...
      // Usar a mensagem da API ou uma mensagem padrão
      const errorMessage =
        responseMessage || 'Não foi possível alterar esta categoria';
  ...
  const inactivateCategory = async (
    ...
    try {
      ...
      const response = await Api.put(
        `core/v1/categorias-financeiro/inativar`,
        categoryIds,
      );
      ...
      const errorMessage =
        responseMessage || 'Não foi possível inativar esta categoria';
  ...
  const activateCategory = async (
    ...
    try {
      ...
      const response = await Api.put(
        `core/v1/categorias-financeiro/ativar`,
        categoryIds,
      );
