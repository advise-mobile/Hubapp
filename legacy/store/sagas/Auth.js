...
export function* login(action) {
  ...
  try {
    ...
    const { idCliente, idUsuarioCliente } = jwtDecode(
      response.data.access_token,
    );

    const isConvenio = contracts.data.itens.some(
      contract => contract.convenioOAB,
    );
    ...
    if (isConvenio) {
      const anotherContract = contracts.data.itens.some(
        contract =>
          (contract.idSituacaoContrato == -1 ||
            contract.idSituacaoContrato == -11) &&
      ...
      if (!anotherContract) {
        ...
        const adheredContract = situation.data.itens[0].planosVinculados.some(
          plan =>
            plan.idSituacaoClienteXPlano === -1 ||
            plan.idSituacaoClienteXPlano === -11,
        );
    ...
    yield put(
      AuthActions.loginSuccess(response.data, isConvenio, active, acceptTerms),
    );
  } catch (err) {
    if (err.response) {
      const errorMessage =
