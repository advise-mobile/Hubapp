...
export default LauchActionsMenu = forwardRef(({item}, ref) => {
	...
	const handleDownloadRelease = useCallback(async () => {
		const downloadPromise = new Promise(async (resolve, reject) => {
			...
			RNFetchBlob.config({
				...
				.catch(err => {
					dispatch(
						ToastNotifyActions.toastNotifyShow('Não foi possível baixar este lançamento', true),
					);
	...
	return (
		<>
			...
			{modalReleaseEditOpen && (
				<ReleaseEdit item={item} ref={modalReleaseEditRef} onClose={closeReleaseEditModal} />
			)}
			{modalReleaseDuplicateOpen && (
				<ReleaseDuplicate
					item={item}
