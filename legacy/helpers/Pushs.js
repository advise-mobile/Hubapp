...
import { PUSH } from '@lhelpers/StorageKeys';
...
const registerNotification = async () => {
  ...
  try {
    ...
    try {
      currentPermission = await OneSignal.Notifications.getPermissionAsync();
    } catch (permError) {
  ...
  await Api.post(`/core/v1/push-notificacao`, {
    ...
    .catch(error => {
      // Erro ao registrar push no servidor
    })
    .finally(() => {
...
const getNotificationSettings = async () => {
  ...
  const { data } = await Api.get(
    `/core/v1/push-notificacao?hash=${hash}&campos=*`,
  );
...
const changeNotificationSettings = async itens =>
  await Api.put(
    `/core/v1/push-notificacao/alterar-situacao-notificacao`,
    itens,
  );
...
const disableNotificationDevice = async () => {
  ...
  const response = await Api.put(
    `/core/v1/push-notificacao/alterar-situacao-dispositivo`,
    {
      itens: [
        {
          hash,
