...
export function* login(action) {
	...
	try {
		...
		const contracts = yield call(
			Api.get,
			'/core/v2/contratos?campos=idSituacaoContrato,convenioOAB&contratosAderidos=true&registrosPorPagina=-1',
		);
		...
		const activeContract = contracts.data.itens.some(
			contract => contract.idSituacaoContrato == -1 || contract.idSituacaoContrato == -11,
		);

		let active = activeContract;

		if (isConvenio) {
			const anotherContract = contracts.data.itens.some(
				contract =>
					(contract.idSituacaoContrato == -1 || contract.idSituacaoContrato == -11) &&
					!contract.convenioOAB,
			);

			if (!anotherContract) {
				const situation = yield call(
					Api.get,
					`/core/v1/clientes?IDs=${idCliente}&campos=planosVinculados.idSituacaoClienteXPlano+&expansao=planosVinculados&registrosPorPagina=-1`,
				);

				const adheredContract = situation.data.itens[0].planosVinculados.some(
					plan => plan.idSituacaoClienteXPlano === -1 || plan.idSituacaoClienteXPlano === -11,
				);
		...
		const {
			...
			Api.get,
			`/core/v1/usuarios-clientes?IDs=${idUsuarioCliente}&campos=aceitaTermosUso&registrosPorPagina=-1`,
		);
...
export function* forgot(action) {
	...
	try {
		...
		yield put(ToastNotifyActions.toastNotifyShow(response.data.status.mensagem, false));
	} catch (err) {
		switch (err.response.status) {
			...
			default:
				yield put(AuthActions.forgotFailure());
				yield put(ToastNotifyActions.toastNotifyShow('Não foi possível recuperar sua senha', true));
		}
	}
}
...
export function* acceptTermsUse({acceptTerms}) {
	try {
		const {
			...
			Api.put,
			'/core/v1/usuarios-clientes/aceitar-termos-uso',
			{},
			{
				redirectLogin: true,
			},
		...
		yield put(
			ToastNotifyActions.toastNotifyShow(
				'Ocorreu uma falha ao aceitar os termos de uso. Por favor tente novamente!',
				true,
			),
		);
