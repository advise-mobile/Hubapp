...
import DeadlineActions from '@lstore/ducks/Deadlines';
...
export function* getDeadlines({ param }) {
  try {
    ...
    // Condição para troca de parametros
    var filteredParams =
      filters === '&concluido=true'
        ? '%2BdataHoraUltAlteracao'
        : '%2BdataHoraInicio';
...
export function* sendDeadlineEmail({ param }) {
  try {
    ...
    yield put(
      ToastNotifyActions.toastNotifyShow('Prazo enviado por email!', false),
    );
...
export function* markAsImportant({ param }) {
  ...
  try {
    ...
    yield put(
      DeadlineActions.deadlinesAlterationSuccess({
        params: { importante },
        id,
      }),
    );
    ...
    yield put(
      ToastNotifyActions.toastNotifyShow(
        `O prazo foi ${importante ? 'marcado' : 'desmarcado'} como importante`,
        false,
      ),
    );
  } catch (error) {
...
export function* markAsConcluded({ param }) {
  ...
  try {
    ...
    yield put(
      DeadlineActions.deadlinesAlterationSuccess({ params: { concluido }, id }),
    );

    yield put(
      ToastNotifyActions.toastNotifyShow(
        `O prazo foi ${concluido ? 'marcado' : 'desmarcado'} como concluído`,
        false,
      ),
    );
  } catch (error) {
...
export function* getTypes() {
  try {
    ...
    const { data } = yield call(
      Api.get,
      `/core/v1/tipos-eventos?campos=*&idCliente=${user.idCliente}`,
    );
