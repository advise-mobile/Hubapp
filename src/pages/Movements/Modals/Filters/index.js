...
export default Filters = forwardRef((props, ref) => {
	...
	const [minDate, setMinDate] = useState(
		props.filters.DataMovimentoInicio ? FormatDateBR(props.filters.DataMovimentoInicio) : null,
	);
	const [maxDate, setMaxDate] = useState(
		props.filters.DataMovimentoFim ? FormatDateBR(props.filters.DataMovimentoFim) : null,
	);
	const [selectedCustom, setSelectedCustom] = useState(props.filters[customField?.name] || 0);

	const countFilters = useCallback(() => checkNull([situation, minDate, maxDate, selectedCustom]), [
		situation,
		minDate,
		maxDate,
		selectedCustom,
			...
			setTimeout(
				() =>
					props.submit({
						DataMovimentoFim: data.DataMovimentoFim || maxDate ? FormatFullDateEN(maxDate) : null,
						DataMovimentoInicio:
							data.DataMovimentoInicio || minDate ? FormatFullDateEN(minDate) : null,
						Lido: radio_props[situation].value,
					}),
				500,
			);
