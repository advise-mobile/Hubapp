...
export default Filters = forwardRef(
  ({ handleSubmitFilters, handleClearFilters }: FilterProps, ref) => {
    ...
    const [quantityDiariesSelected, setQuantityDiariesSelected] =
      useState<number>(0);
    ...
    const [quantityJournalsSelected, setQuantityJournalsSelected] =
      useState<number>(0);
    ...
    const [idTipoMovProcesso, setIdTipoMovProcesso] = useState<number | null>(
      null,
    );
    const [minDate, setMinDate] = useState<Date | string | null>(null);
    const [maxDate, setMaxDate] = useState<Date | string | null>(null);
    ...
    const onSubmit = (data: DataFilterProps) => {
      // Garante que os valores das datas sejam coletados corretamente
      // Usa os estados locais como fallback caso o form não tenha os valores
      const dataInicio = minDate
        ? FormatFullDateEN(minDate)
        : data.DataMovimentoInicio || getValues('DataMovimentoInicio') || null;
      const dataFim = maxDate
        ? FormatFullDateEN(maxDate)
        : data.DataMovimentoFim || getValues('DataMovimentoFim') || null;

      const formData: DataFilterProps = {
        DataMovimentoInicio: dataInicio || '',
        DataMovimentoFim: dataFim || '',
        idPalavraChave: data.idPalavraChave || keyWordsCheckeds || [],
        idDiario: data.idDiario || dataDiariesCheckeds || [],
        idJournals: data.idJournals || dataJournalsCheckeds || [],
        idTipoMovProcesso:
          data.idTipoMovProcesso ||
          (idTipoMovProcesso ? idTipoMovProcesso.toString() : ''),
        Lido:
          data.Lido !== undefined && data.Lido !== null
            ? data.Lido
            : ((situation === 0
                ? null
                : situation === 1
    ...
    useEffect(() => {}, [isLoadingDiaries]);

    // - monitora ação de buscar cadernos
    useEffect(() => {}, [isLoadingJournals]);
    ...
    const renderTypesMovimentation = () => {
      ...
      const onValidateType = (value: string | null) => {
        if (value === '-1' || value === '-4') {
          // - reset keywords
          setQuantitySelected(0);
          setKeyWordsCheckeds([]);
                ...
                <Column>
                  <RNPickerSelect
                    ...
                    onValueChange={value => {
                      ...
                      setValue('idTipoMovProcesso', value);
                      onValidateType(value);
                    }}
                    placeholder={{}}
                    doneText="Selecionar"
                    value={
                      idTipoMovProcesso !== null &&
                      idTipoMovProcesso !== undefined
                        ? idTipoMovProcesso
                        : null
                    }
                    items={data}
      ...
      return (
        <ContainerMultiSelect>
          <SpaceRow>
            ...
            <ClearButtom onPress={() => onChangeSelected([])}>
              <ClearButtomText>Limpar</ClearButtomText>
            </ClearButtom>
          </SpaceRow>
          ...
          {loadingKeyWords ? (
            ...
            <>
              <ContainerSubtitle onPress={() => setShowKeyWords(!showKeyWords)}>
                <SpaceRow>
                  <SubTitleMultiSelectText>
                    {quantitySelected} Palavra-chave(s) selecionada(s)
                  </SubTitleMultiSelectText>
                  <Label>
                    <MaterialIcons
                      name="arrow-drop-down"
      ...
      return (
        <ContainerMultiSelect>
          ...
          <ContainerSubtitle onPress={() => handleGetDiaries()}>
            <SpaceRow>
              <SubTitleMultiSelectText>
                {quantityDiariesSelected} Diário(s) selecionado(s)
              </SubTitleMultiSelectText>
              <Label>
      ...
      return (
        <ContainerMultiSelect>
          ...
          <ContainerSubtitle onPress={() => handleGetJournals()}>
            <SpaceRow>
              <SubTitleMultiSelectText>
                {quantityJournalsSelected} Caderno(s) selecionado(s)
              </SubTitleMultiSelectText>
              <Label>
                <MaterialIcons name="arrow-drop-down" size={18} color="gray" />
              </Label>
    ...
    const handleModalOpen = useCallback(() => {
      // Carrega Keywords apenas quando o modal é aberto pela primeira vez
      if (dataKeyWords.length === 0 && !loadingKeyWords) {
        getKeyWordsData();
      }
    }, [dataKeyWords.length, loadingKeyWords, getKeyWordsData]);
            ...
            <Controller
              ...
              render={() => (
                <Datepicker
                  ...
                  onDateChange={date => {
                    setMinDate(date);
                    setValue('DataMovimentoInicio', FormatFullDateEN(date));
                  }}
                />
              )}
            />
            ...
            <Controller
              ...
              render={() => (
                <Datepicker
                  ...
                  onDateChange={date => {
                    setMaxDate(date);
                    setValue('DataMovimentoFim', FormatFullDateEN(date));
                  }}
                />
              )}
            />
