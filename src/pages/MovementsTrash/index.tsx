...
export default MovementsTrash = () => {
  ...
  const { movementsTrash, loading, getData } = useGetMovementsTrash({
    page: currentPage,
  });
  ...
  const closeOpenedRow = useCallback(
    (item: ItemProps) => listRef.current?._rows[item.id].closeRow(),
    [],
  );
  ...
  const handleModalRecoverCancel = useCallback(
    () => confirmationModalRecoverRef.current?.close(),
    [],
  );
  ...
  const handleModalRecoverSubmit = useCallback(async () => {
    if (currentItem) {
      const recover = await recoverMoviment(
        currentItem,
        handleModalRecoverCancel,
      );
      if (recover) {
        await getData({
          page: 1,
          ...
          >
                ...
                {item.palavrasChaves?.map(
                  keyword =>
                    keyword.idPalavraChavePrincipal === undefined && (
                      <Tag
                        background={item.lido ? colors.gray : colors.green}
                        key={keyword.id}
                      >
            ...
            {item.idTipoMovProcesso === -4 && (
              <>
                <Tag background={item.lido ? colors.gray : colors.orange200}>
                  <TagText>
                    {item.nomeTipoMovProcesso || 'Processo INSS'}
                  </TagText>
                </Tag>
                {item.numeroProcesso && (
                  <Tag background={colors.gray}>
                ...
                {item.nomeProcesso && (
                  <Tag background={colors.gray}>
                    <TagText>Proc.: {item.nomeProcesso}</TagText>
                  </Tag>
                )}
              </>
            )}
