...
import { GetMonthPeriod } from '@lhelpers/DateFunctions';
...
export default function Finance(props) {
  ...
  const renderTabs = useCallback(
    () =>
      tabs.map((item) => (
        <FilterTabItem key={item.id}>
          <FiltersButton onPress={() => handleFilter(item.id)}>
            <FiltersText active={currentTab === item.id} numberOfLines={1}>
              {item.name}
            </FiltersText>
            ...
            {loading ? (
              ...
              <>
                ...
                {currentTab === 'category' && (
                  <Category
                    ref={categoryRef}
                    dataFiltersCategory={dataFiltersCategory}
                  />
                )}
              </>
            )}
