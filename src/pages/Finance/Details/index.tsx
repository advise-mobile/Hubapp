...
import { Container } from '@lassets/styles/global';
...
export default function Details(props) {
  const { isLoadingInstallmentsDetails, getInstallmentsDetails } =
    useGetInstallmentsDetails();

  const { item } = props.route.params;

  const [dataDetails, setDataDetails] =
    useState<ItemsInstallmentsDetailsProps>();

  const [itemComplete, setItemComplete] = useState(item);

  const LauchActionsMenuRef = useRef(null);

  const { addFinancialLoss, removeFinancialLoss, isLoadingFinancialLoss } =
    useFinancialLoss();
    ...
    try {
      ...
      const itemComplete = {
        ...
        idTipoParcelamentoFinanceiro:
          installmentsDetails?.idTipoParcelamentoFinanceiro,
        observacao: installmentsDetails?.observacao,
        idProcesso: installmentsDetails?.idProcesso || null,
        idPessoaCliente: installmentsDetails?.idPessoaCliente || null,
        idLancamentoFinanceiro:
          installmentsDetails?.idLancamentoFinanceiro ||
          item.idLancamentoFinanceiro ||
          null,
        categoriaFinanceiro: installmentsDetails?.categoriaFinanceiro ||
          item.categoriaFinanceiro || {
            idCategoriaFinanceiro:
              installmentsDetails?.idCategoriaFinanceiro || null,
            nomeCategoriaFinanceiro:
              installmentsDetails?.categoria || item.category || null,
          },
      };
      // Logs removidos para melhorar performance em debug mode
      // console.log(
      //   '📋 Details - installmentsDetails recebido:',
      //   JSON.stringify(installmentsDetails, null, 2),
      // );
      // console.log('📋 Details - item original:', JSON.stringify(item, null, 2));
      // console.log(
      //   '📋 Details - itemComplete montado:',
      //   JSON.stringify(itemComplete, null, 2),
      // );
    ...
    <Container>
      ...
      <ContainerScreen>
        ...
        <InformationContainer>
          ...
          <InformationTextContainer>
            <InformationText>
              {item.dataVencimentoFormatada || 'N/I'}
            </InformationText>
          </InformationTextContainer>
