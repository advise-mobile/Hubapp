...
import { Actions, ActionButton } from '@lassets/styles/global';
...
import {
  useGetCategory,
  useCategory,
} from '@services/hooks/Finances/useCategory';
import {
  CategoryProps,
  DataFiltersCategory,
  CategoryRef,
...
const Category = forwardRef<CategoryRef, DataFiltersCategory>((props, ref) => {
  ...
  const { isSavingCategory, inactivateCategory, activateCategory } =
    useCategory();
  ...
  const handleModalCancel = useCallback(
    () => confirmationInativationModalRef.current?.close(),
    [],
  );

  const handleModalInativationSubmit = useCallback(async () => {
    if (currentItem) {
      const inativation = await inactivateCategory(
        currentItem,
        handleModalCancel,
      );
      if (inativation) {
  ...
  const renderConfirmationInativation = useMemo(
    () => (
      <ConfirmModal
        ...
        onSubmit={
          currentItem?.ativo
            ? handleModalInativationSubmit
            : handleModalActivateSubmit
        }
        cancelText="Cancelar"
        submitText={
          currentItem?.ativo ? 'Sim, quero inativar ' : 'Sim, quero ativar'
        }
  ...
  return (
    <>
      {isInitialLoading || (isLoadingCategory && dataResume.length === 0) ? (
        <ContainerSpinner>
        ...
        <SwipeListView
          ...
          keyExtractor={(item: CategoryProps) =>
            item.idCategoriaFinanceiro?.toString() || ''
          }
      ...
      {categoryEdit && (
        <EditCategory
          item={categoryEdit}
          ref={editCategoryRef}
          onClose={handleModalEditCategory}
        />
      )}
