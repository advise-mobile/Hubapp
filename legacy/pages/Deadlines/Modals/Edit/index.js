...
export default Edit = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    ...
    setValue('titulo', props.deadline?.titulo || '');
    setValue('diaInteiro', diaInteiroValue);
    setValue('data', FormatFullDateEN(
      props.deadline?.dataHoraInicio
        ? moment(props.deadline.dataHoraInicio).toDate()
        : new Date()
    ));
    ...
    setValue('localizacao', props.deadline?.localizacao || '');
    setValue('observacao', props.deadline?.observacao || '');

    setDefaultValues({
      titulo: props.deadline?.titulo || '',
      diaInteiro: diaInteiroValue,
      data: props.deadline?.dataHoraInicio
        ? moment(props.deadline.dataHoraInicio).toDate()
        : new Date(),
  ...
  // Observa quando a edição é concluída e recarrega a lista
  useEffect(() => {
    if (triggerChange && !processing && props.onEditSuccess) {
      // Aguarda um pouco para garantir que o toast foi exibido
      setTimeout(() => {
        props.onEditSuccess();
      }, 1000);
    }
  }, [triggerChange, processing, props.onEditSuccess]);
              ...
              render={() => (
                <Hour onPress={() => setViewPicker(true)}>
                  ...
                  <DateTimePickerModal
                    ...
                    onConfirm={date => {
                      const horaFormatada = handleConfirm(date);
                      setValue('hora', horaFormatada);
