...
export default Confirmation = forwardRef((props, ref) => {
    ...
    <Footer>
      ...
      <Submit onPress={() => confirm()} disabled={sending}>
        {sending ? (
          ...
          <SubmitText>Sim, quero excluir</SubmitText>
        )}
      </Submit>
    </Footer>
  );
  ...
  return (
    <Modal
      ref={ref}
      title="Deseja excluir o prazo?"
      footer={footer()}
      maxHeight={250}
    >
      <Content>
        <Message>
          Ao excluir um prazo, você elimina todas as informações referentes a
          este prazo. A ação de excluir é definitiva e irreversível.
        </Message>
      </Content>
    </Modal>
  );
