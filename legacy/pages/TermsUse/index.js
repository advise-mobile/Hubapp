...
const TermsUse = props => {
  ...
  const fromPageInfoParams =
    props.route.params && props.route.params.previous_screen ? true : false;

  const [fromPageInfo, setFromPageInfo] = useState(fromPageInfoParams);

  const acceptTerms = useSelector(state => state.auth.acceptTerms);
  const loadingAcceptTerms = useSelector(
    state => state.auth.loadingAcceptTerms,
  );
  ...
  useEffect(() => {
    async function getNewTermo() {
      ...
      const { data } = await Axios.get(
        `${BASE_URL}/core/v1/termos/termo-uso-vigente`,
        {
          headers,
        },
      );
      ...
      <S.Container>
        <S.ContentWrapper>
          ...
          <S.TextWrapper>
            ...
            />
            <S.Title style={{ fontSize: 20, marginBottom: 28 }}>
          ...
          {fromPageInfo ? (
            <>
              <S.AcceptTermsWrapper>
                <S.TermsText color={colors.white}>
                  Termo de uso já aceito anteriormente
                </S.TermsText>
              </S.AcceptTermsWrapper>
              <S.AcceptButton onPress={handleGoback} activeOpacity={0.7}>
