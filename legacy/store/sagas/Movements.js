...
function getFilterString(filters) {
  ...
  Object.keys(filters).map(
    key =>
      (result += (filters[key] != null && `${key}=${filters[key]}&`) || ''),
  );
...
export function* getMovements({ params }) {
  try {
    ...
    const optmizedMovements = data.itens.map(movement => {
      ...
      if (andamentoProcesso) {
        ...
        optmizedMovement = {
          ...
          title: `${formatedDate} - ${siglaOrgaoJudiciario} - ${
      ...
      if (publicacao) {
        ...
        if (processosPublicacoes?.length > 0) {
          if (publicacao.numero) {
            ...
            optmizedMovement.numeroProcesso = MaskCnj(
              processosPublicacoes[0].numeroProcesso,
            );
          }
        }
      }
    ...
    if (refreshing)
      yield put(
        MovementsTypes.movementsRefreshSuccess(
          optmizedMovements,
          params.page,
          endReached,
        ),
      ...
      yield put(
        MovementsTypes.movementsSuccess(
          optmizedMovements,
          params.page,
          endReached,
        ),
...
export function* sendMovementsEmail({ param }) {
  try {
    ...
    yield put(
      ToastNotifyActions.toastNotifyShow(
        'Movimentação enviada por email!',
        false,
      ),
    );
...
export function* deleteLogicalMovement({ params }) {
  try {
    ...
    yield call(
      Api.put,
      `/core/v1/movimentos-usuarios-clientes/enviar-lixeira`,
      data,
    );
