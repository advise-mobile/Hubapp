...
const notFound =
	colorScheme == 'dark'
		? require('assets/images/not_found/info_white.png')
		: require('assets/images/not_found/info.png');

export default function jurisprudenceList(props) {
	const filtersRef = createRef();

	const [filters, setFilters] = useState({});
	const [term] = useState(props.route.params.term);
	const [reporter] = useState(props.route.params.reporter);
	const [resource] = useState(props.route.params.resource);
	...
	const renderers = {
		description: (htmlAttribs, children, convertedCSSStyles, passProps) => (
			<Description key={Math.random()} numberOfLines={7}>
				{children}
			</Description>
		),
	...
	const renderItem = ({item}) => {
		...
		return (
			<Jurisprudence
				key={codEmenta}
				onPress={() =>
					props.navigation.navigate('JurisprudenceDetail', {jurisprudence: item, term})
				}>
				...
				{dataPublicacao ? (
					...
					<PublicationDate>Data de publicação: Não informado</PublicationDate>
				)}
				<HTML
					source={{
						html: `<title>${item.tituloMarcado || item.titulo}</title><description>${
							item.ementaMarcada?.join('').split('\u0000').join('') || item.ementa
						}</description>`,
					}}
					renderers={renderers}
					ignoredTags={['strong']}
