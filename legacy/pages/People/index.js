...
import PeopleActions from '@lstore/ducks/People';
...
import {Container, Warp, Actions, ActionButton} from '@lassets/styles/general';
...
export default People = props => {
		...
		return (
			<ListItem onPress={() => {}} underlayColor={colors.white} activeOpacity={1}>
				<ListContainer>
					...
					/>
					<UserIcon
						name={data.item.nomePessoaCliente}
	...
	const renderBadges = useCallback(
		data => {
			return data.contato[0].contatoXMarcadorContato
				.filter(marker => marker.ativo)
				.map((activeMarker, index) => {
					return (
						<Badge key={index}>
		...
		<Actions>
			...
			<ActionButton onPress={() => {}}>
				<MaterialIcons
					name="block"
					size={24}
					color={data.item.contato[0].ativo ? colors.fadedBlack : colors.red}
				/>
			</ActionButton>
		</Actions>
	));
