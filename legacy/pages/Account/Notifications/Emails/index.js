...
import HeaderGlobals from '@lcomponents/HeaderGlobals';
...
const Emails = props => {
  ...
  // Estados locais para controlar seleção dos radio buttons
  const [selectedMovements, setSelectedMovements] = useState(null); // -24 ou -25
  const [selectedPublications, setSelectedPublications] = useState(null); // -23 ou -1
  const [selectedProcesses, setSelectedProcesses] = useState(null); // -31 ou -3
  ...
  const getEmailsUser = useCallback(() => {
    ...
    Api.get(`/core/v1/emails-usuario-cliente?campos=*&flAtivo=true`).then(
      ({ data }) => {
        const { itens } = data;
  ...
  const getActionEvents = useCallback(() => {
    ...
    Api.get(
      ...
      .then(response => {
        ...
        setData(
          itens.filter(item => item.nomeProdutoAdviseXAcaoEvento == 'HUB'),
        );
        ...
        if (ids && (ids.includes(-24) || ids.includes(-25))) {
          ...
          if (ids.includes(-24)) {
            setSelectedMovements(-24);
          } else if (ids.includes(-25)) {
            setSelectedMovements(-25);
          }
        } else {
        ...
        if (ids && (ids.includes(-1) || ids.includes(-23))) {
          ...
          // Define qual opção está selecionada
          if (ids.includes(-23)) {
            setSelectedPublications(-23);
          } else if (ids.includes(-1)) {
            setSelectedPublications(-1);
          }
        } else {
        ...
        if (ids && (ids.includes(-3) || ids.includes(-31))) {
          ...
          // Define qual opção está selecionada
          if (ids.includes(-31)) {
            setSelectedProcesses(-31);
          } else if (ids.includes(-3)) {
            setSelectedProcesses(-3);
          }
        } else {
  ...
  const handleNoNewPublications = () => {
    ...
    if (!active) {
      newValues = data.map(action => {
        if (
          action.idRegraAcaoEvento == -24 ||
          action.idRegraAcaoEvento == -25
        ) {
          action.update = true;
          action.ativo = false;
        }
  ...
  const handleNotificationMovements = useCallback((key, value) => {
    // Atualiza estado local para controle visual imediato
    if (key === -1 || key === -23) {
      setSelectedPublications(key);
    } else if (key === -3 || key === -31) {
      setSelectedProcesses(key);
    } else if (key === -24 || key === -25) {
      setSelectedMovements(key);
    }

    // Atualiza dados para envio ao backend
    setData(prevData => {
      return prevData.map(action => {
        // Grupo de publicações (-1, -23)
        if (key === -1 || key === -23) {
          if (
            action.idRegraAcaoEvento == -1 ||
            action.idRegraAcaoEvento == -23
          ) {
            return {
              ...action,
              update: true,
              ativo: action.idRegraAcaoEvento == key,
            };
          }
        }
        // Grupo de andamentos (-3, -31)
        else if (key === -3 || key === -31) {
          if (
            action.idRegraAcaoEvento == -3 ||
            action.idRegraAcaoEvento == -31
          ) {
            return {
              ...action,
              update: true,
        ...
        // Retorna novo objeto mesmo quando não há mudança para garantir re-render
        return { ...action };
      });
    });
  }, []);
  ...
  const onRecordEmail = useCallback(
    email => setEmails([email, ...emails]),
    [emails],
  );
          ...
          >
            ...
            {permissionPublications && (
              <>
                <ListItem>
                  <ListContent>
                    <ListText>
                      Notificar por email quando não houver novas publicações
                    </ListText>
                    <Switch
                      ...
                      <RadioForm animation={true} style={{ flex: 1 }}>
                        <Option as={RadioButton}>
                          <RadioButtonInput
                            ...
                            isSelected={selectedMovements === -24}
                            onPress={() => {
                              handleNotificationMovements(-24, -24);
                            }}
                            borderWidth={1}
                          ...
                          <View>
                            ...
                            >
                              {colorUseTheme.name === 'dark' ? (
                                ...
                                <Image
                                  source={require('assets/images/icons/info_icon.png')}
                                />
                              )}
                            </TouchableOpacity>
                            <RadioButtonLabel
                        ...
                        <Option as={RadioButton}>
                          <RadioButtonInput
                            ...
                            isSelected={selectedMovements === -25}
                            onPress={() => {
                              handleNotificationMovements(-25, -25);
                            }}
                            borderWidth={1}
                            buttonInnerColor={colors.primary}
                            buttonOuterColor={colors.primary}
                ...
                <ListItem>
                  <ListContent>
                    <ListText>
                      Notificar por email quando houver novas publicações
                    </ListText>
                    <Switch
                      ...
                      <RadioForm animation={true} style={{ flex: 1 }}>
                        <Option as={RadioButton}>
                          <RadioButtonInput
                            ...
                            isSelected={selectedPublications === -23}
                            onPress={() => {
                              handleNotificationMovements(-23, -23);
                            }}
                            borderWidth={1}
                            buttonInnerColor={colors.primary}
                            buttonOuterColor={colors.primary}
                        ...
                        <Option as={RadioButton}>
                          <RadioButtonInput
                            obj={{
                              label:
                                'Enviar apenas o aviso de novas publicações',
                              value: -1,
                            }}
                            isSelected={selectedPublications === -1}
                            onPress={() => {
                              handleNotificationMovements(-1, -1);
                            }}
                            borderWidth={1}
                            buttonInnerColor={colors.primary}
                            buttonOuterColor={colors.primary}
            ...
            {permissionProcesses && (
              <ListItem>
                <ListContent>
                  <ListText>
                    Notificar por email quando houver novos andamentos
                  </ListText>
                  <Switch
                    ...
                    <RadioForm animation={true} style={{ flex: 1 }}>
                      <Option as={RadioButton}>
                        <RadioButtonInput
                          obj={{
                            label:
                              'Enviar os processos que receberam novos andamentos',
                            value: -31,
                          }}
                          isSelected={selectedProcesses === -31}
                          onPress={() => {
                            handleNotificationMovements(-31, -31);
                          }}
                          borderWidth={1}
                          buttonInnerColor={colors.primary}
                          buttonOuterColor={colors.primary}
                      ...
                      <Option as={RadioButton}>
                        <RadioButtonInput
                          ...
                          isSelected={selectedProcesses === -3}
                          onPress={() => {
                            handleNotificationMovements(-3, -3);
                          }}
                          borderWidth={1}
                          buttonInnerColor={colors.primary}
                          buttonOuterColor={colors.primary}
