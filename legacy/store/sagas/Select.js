...
import SelectActions from '@lstore/ducks/Select';
  ...
  try {
    ...
    if (type === 'event-type') {
      ...
      const { data } = yield call(
        Api.get,
        `/core/v1/tipos-eventos?campos=*&idCliente=${idCliente}`,
      );
      data.itens.forEach(value =>
        list.push({
          title: value.nome,
          id: value.id,
        }),
    ...
    yield put(
      SelectActions.fetchSelectOptionsFailure(
