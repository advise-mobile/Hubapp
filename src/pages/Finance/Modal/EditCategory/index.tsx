...
export default EditCategory = forwardRef((props, ref) => {
  ...
  const [nomeCategoriaFinanceiro, setNomeCategoriaFinanceiro] =
    useState<string>();
  const [idTipoCategoriaFinanceiro, setIdTipoCategoriaFinanceiro] =
    useState<number>();
  ...
  useEffect(() => {
    ...
    setIdTipoCategoriaFinanceiro(
      item.tipoCategoriaFinanceiro.idTipoCategoriaFinanceiro,
    );
    setCorCategoria(item.corCategoria);

    setValue('nomeCategoriaFinanceiro', item.nomeCategoriaFinanceiro);
    setValue(
      'idTipoCategoriaFinanceiro',
      item.tipoCategoriaFinanceiro.idTipoCategoriaFinanceiro,
    );
            ...
            render={({ field }) => (
              <Input
                ...
                placeholderTextColor={
                  errors.nomeCategoriaFinanceiro
                    ? colors.red200
                    : colors.grayLight
                }
                returnKeyType="next"
                onChangeText={value => {
                  setValue('nomeCategoriaFinanceiro', value, {
                    shouldValidate: true,
                  });
                ...
                <RBRow as={RadioButton} key={i}>
                  <RadioButtonInput
                    ...
                    onPress={() => {
                      ...
                      setValue('idTipoCategoriaFinanceiro', obj.value, {
                        shouldValidate: true,
                      });
                    }}
                    borderWidth={1}
                  ...
                  <RadioButtonLabel
                    isError={
                      errors.idTipoCategoriaFinanceiro &&
                      idTipoCategoriaFinanceiro === null
                    }
                    obj={obj}
                    labelStyle={[
                      RBLabel.label,
              ...
              <ContainerColor>
                {colorData.map(({ id, color }) => (
                  <ColorsItem
                    ...
                    onPress={() => {
                      setValue('corCategoria', color, { shouldValidate: true });
