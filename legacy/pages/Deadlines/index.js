...
export default function Deadlines(props) {
	...
	const [currentDeadline, setCurrentDeadline] = useState(deadlinesData[0]);
  ...
  */
	...
	/*
  useEffect(() => {
    deadlinesData.map((_, i) => (rowTranslateAnimatedValues[i] = new Animated.Value(1)));
  }, [deadlinesData]);
  */
	...
	}, [deadlinesData, handleMonthChange, selectedMonth, theme]);

	const getMarkedDates = useCallback(() => {
		let datas = {};

		deadlinesData.map(prazo => {
				...
				<>
					{havePermission ? (
						<Warp>
							...
							/>
							{/* <Header title="Prazos" filter={deadlinesData.length > 0 || Object.keys(customFilters).length > 0 ? () => filtersRef.current?.open() : null} /> */}
							<Filters
